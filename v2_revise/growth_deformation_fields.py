...
class GrowthAwareVelocityField3d(BsplineTemporalIntegratedVelocityField3d):
    ...
    def __init__(self, config: GrowthAwareVelocityField3dConfig) -> None:
        ...
        # Zero-init last layer (GrowFlow's init_as_identity pattern)
        # Ensures G(x,t)=0 at training start → growth_accum=0 → exp(0)=1
        # → density = canonical (identity, safe starting point)
        with torch.no_grad():
    ...
    def _eval_derivs(self, x0, x1, x2, t, enable_growth):
        ...
        Factored out for RK4's 4 evaluations per step."""
    ...
    def _ode_step(self, x0, x1, x2, g_acc, t, dt, enable_growth):
        ...
        4th-order Runge-Kutta: local error O(dt^5), vs Euler's O(dt^2).
        GrowFlow uses dopri5 (adaptive RK45) via torchdiffeq — RK4 is the
        closest fixed-step equivalent within B-spline framework.
        
        4 network evaluations per step, but can use 2x larger timedelta
        for same accuracy, so total compute is comparable.
        """
