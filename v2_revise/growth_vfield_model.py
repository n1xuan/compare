...
class GrowthVfieldModel(Model):
    ...
    def _apply_growth_activation(self, growth_accum: Tensor) -> Tensor:
        """Convert growth accumulation to multiplicative attenuation factor.
        
        Multiplicative model: density = canonical × exp(-ReLU(growth_accum))
        
        Properties:
        - growth_accum = 0  → factor = 1.0 (no change, correct at t=T)
        - growth_accum = 1  → factor = 0.37 (63% density reduction)
        - growth_accum = 3  → factor = 0.05 (95% density reduction)
        - factor is ALWAYS > 0 → density is always non-negative
        - gradient is ALWAYS nonzero → no dead zones (unlike relu-subtract)
        
        Physical interpretation: growth_accum acts as an absorption/attenuation
        coefficient. Larger growth = more attenuation = less density at early frames.
        """
        return torch.exp(-torch.nn.functional.softplus(growth_accum, beta=20.0))
    ...
    def get_outputs(self, ray_bundle: RayBundle, **kwargs):
        ...
        # Apply multiplicative growth attenuation to density
        if 'accum' in growth_storage and growth_storage['accum'] is not None:
            growth_accum = growth_storage['accum']
            # exp(-ReLU(growth_accum)): returns factor in (0, 1]
            growth_factor = self._apply_growth_activation(growth_accum)
            growth_factor = growth_factor.to(
                field_outputs[FieldHeadNames.DENSITY].dtype
            )
            field_outputs[FieldHeadNames.DENSITY] = (
                field_outputs[FieldHeadNames.DENSITY]
                * growth_factor.unsqueeze(-1)
            )
            # No relu needed: exp() output is always > 0
    ...
    def get_density_from_pos(
        ...
        rho(x,t) = rho_canonical(warp(x, t->T)) × exp(-ReLU(growth_accum))
        """
        if time is None:
        ...
        with cm():
            ...
            if growth_accum is not None:
                growth_factor = self._apply_growth_activation(growth_accum)
                growth_factor = growth_factor.to(density.dtype)
                density = density * growth_factor
